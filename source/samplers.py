...
def get_laion_dataset(train_xform, tup_file='result_tup_list', val_split=0, val_xform=None):
    imgs = torch.load(tup_file)
    for img, _ in imgs:
    ...
    if val_split > 0:
        ...
        return BaseDataset(train_imgs, train_xform), BaseDataset(val_imgs, val_xform)
        
    return BaseDataset(imgs, train_xform)
