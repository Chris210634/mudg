...
#         self.vocab_size = open_clip_model.vocab_size
        ...
        ### CLIP-Adapter
        if not args is None and args.adapter:
            self.adapter = Adapter(args.d, reduction=args.rank)
        else:
            self.adapter = None
            
        ### Regular Adapter
        if not args is None and args.resblock_adapter:
            # vision backbone
        ...
        ### SSF
        if not args is None and args.ssf:
            # vision backbone
            for depth, resblock in enumerate(open_clip_model.visual.transformer.resblocks):
                if depth >= args.layer_start_v:
        ...
        ### LoRA
        if not args is None and args.lora:
            for depth, resblock in enumerate(open_clip_model.visual.transformer.resblocks):
        ...
        if not args is None and args.text_prompt_depth > 0:
        ...
        if not args is None and args.maple:
    ...
    def encode_image(self, image, normalize: bool = False):
        ...
        x = torch.cat([_expand_token(self.visual.class_embedding, x.shape[0]).to(x.dtype), x], dim=1) # shape = [*, grid ** 2 + 1, width]
