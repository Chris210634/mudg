...
dset_base_train, dset_base_val = get_laion_dataset(
    train_xform, 
    tup_file=args.tup_file, 
    val_split=args.val_split,
    val_xform=test_xform)
...
print('len(dset_base_val): ', len(dset_base_val))
...
for epoch in range(epochs):
    ...
    if args.dataset == 'DomainNet':
        dataset_list = domainnet_datasets
    elif args.dataset == 'OfficeHome':
        dataset_list = officehome_datasets
    elif args.dataset == 'VLCS':
        dataset_list = VLCS_datasets
    elif args.dataset == 'PACS':
        dataset_list = PACS_datasets
    elif args.dataset == 'TerraInc':
        dataset_list = TerraInc_datasets
    else:
    ...
    for dataset in dataset_list:
        datasetname = args.dataset if dataset == 'validation' else dataset
        ...
        dataset_class = dataset_classes[datasetname]
        ...
        prompt = prompt_strings[datasetname]
...
#         image_features, y_truth = get_features(dl_test, model_copy, d=args.d)
        ...
        ## Change this back
        fn = 'cache/image_features.y_truth.{}{}{}.tup'.format(datasetname, modelname, pretrained)
    
        if fn.split('/')[-1] in os.listdir('cache') and (args.eval_only or args.use_cached_image_features) and not dataset == 'validation':
            try:
            ...
            with torch.no_grad():
                image_features, y_truth = get_features(dl_test, model_copy, d=args.d)
            if (args.eval_only or args.use_cached_image_features) and not dataset == 'validation':
                torch.save((image_features, y_truth), fn)
        ##################################
...
#             sys.exit(1)
        ...
        if args.openai_eval:
            text_features = gpt_helpers.get_openai_manual_prompt_template_centroids(
                dset.classnames, model_copy, datasetname, tokenizer 
        ...
        if args.gpt_centroid_eval:
            gpt_descriptions = gpt_helpers.get_gpt_descriptions(dataset=datasetname)
