...
#     score_A = (var_bases - vars_master).mean(0)
    
#     model.eval()
#     score_B = (F.normalize(zs_text_features.cuda()) @ _get_description_features_without_label_names(
#         model, descriptions).T.cuda()).mean(dim=0).cpu()

    indices = (vars_master < var_bases).float().sum(0).sort(descending=True).indices
    torch.save((vars_master < var_bases).float().sum(0).sort(descending=True).values, 'tmp')
#     indices = score_B.sort(descending=True).indices
    return indices
